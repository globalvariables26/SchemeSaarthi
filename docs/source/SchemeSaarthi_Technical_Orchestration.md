...
```
Each node:
Reads only the slice of shared state it's allowed to (enforced per Section 6.3 for the
Consent & Profile Agent specifically).
Does its work (LLM call, DB query, data.gov.in call, etc.).
Writes its result — including a `status` field (`ok` / `degraded` / `failed`) and a
human-readable `reason` string — back to shared state.
Returns control to the Orchestrator routing function, which reads that `status` field to
...
each read, not cache it — cheapest way to do this is to make `ConsentGateway.get(attribute, purpose)` the only getter, with no attribute value ever copied into another agent's local
...
and require a structured JSON response: `{applies: bool, confidence: 0-1, missing_data: [...], reasoning: str}`. Use Groq's JSON mode / tool-call response format (`response_format: {"type": "json_object"}`, or Groq's native tool-calling schema) so you get valid JSON back,
...
4.7 `audit_log`
`id`, `citizen_id (fk)`, `agent_name`, `action`, `justifying_attribute_ids (jsonb array, nullable)`, `reasoning (text)`, `status`, `created_at`
...
7.2 Mechanics
Register a free account at data.gov.in and generate an API key.
Base pattern: `https://api.data.gov.in/resource/<resource_id>?api-key=<key>&format=json &limit=<n>&filters[<field>]=<value>`
Do not hardcode resource IDs from this document — I was not able to independently
...
Concrete search terms to use directly on data.gov.in's catalog search (not a general web
...
Once a working resource_id is found, build a small `DataGovInClient` wrapper (Python,
...
Map whatever fields the found dataset actually has (e.g. state-wise household/income
...
---
9. Deployment plan
Frontend → Vercel, connected to the GitHub repo, auto-deploy on push to `main`.
Backend (FastAPI + LangGraph) → Railway or Render free tier, connected the same way.
Needs a `Procfile` or equivalent start command (`uvicorn main:app --host 0.0.0.0 --port $PORT`).
Database/auth → Supabase Cloud project (free tier) — same project used by both frontend
...
`scheme_source`).
Confirm CORS is configured on FastAPI to allow the deployed Vercel domain.
Test the full failure-injection demo path on the deployed version, not just locally —
...
Supabase project + schema — create all tables from Section 4, set up RLS policy on
`citizen_attributes` first (this is foundational, not a later add-on).
Seed data — synthetic citizen(s), schemes (including the deliberate conflict pair and
state/central mismatch), document rules. Do this before building agents so there's real
data to test against immediately.
`ConsentGateway` + Consent & Profile Agent — build and test attribute-read gating in
isolation before anything else depends on it.
LangGraph skeleton — all 7 nodes as stubs that just pass state through, Orchestrator
routing logic wired up, confirm the graph runs end-to-end with stub nodes.
Discovery + Matching + Document agents — real logic, tested against seeded data.
Conflict & Explainability agent — needs the above three working first since it reads
their output.
Notification agent + SSE event stream — wire up real-time visibility.
FastAPI endpoints — expose the graph and admin actions.
Frontend: admin panel + dashboard first (per Section 8's build-order note), then the
remaining four surfaces.
data.gov.in integration — do this in parallel with step 9 once a real working resource
ID is confirmed (Section 7.4); it's not a blocker for anything else.
Full failure-injection rehearsal on the deployed version (Section 10).
Audit log UI + why-not-matched explorer — last, since they're judged on depth/polish
